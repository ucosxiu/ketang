...
function Detector(a) {
    ...
    this.detect = function (response) {
        var b = new _cc()._ce(this.image, response);
        return this._cd(b)
    }
}
var _ca = 21522;
...
qrcode.decode = function (d, response) {
    if (arguments.length == 0) {
        ...
        c.onload = function () {
            ...
            try {
                ...
                if (qrcode.callback != null) {
                    qrcode.callback(qrcode.result, response)
                }
                return
            }
            try {
                qrcode.result = qrcode.process(h, response)
            } catch (m) {
                console.log(m);
        ...
        c.onerror = function () {
            if (qrcode.callback != null) {
                qrcode.callback("Failed to load the image", response)
            }
        };
        c.src = d
    }
...
qrcode.process = function (q, response) {
...
function _cc() {
    ...
    this._ee = function (response) {
        var h = this._cv.length;
        if (h < 3) {
            if (qrcode.empty) {
                qrcode.empty(response)
            }
