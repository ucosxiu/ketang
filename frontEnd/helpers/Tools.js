...
const key = 'contentWindowHig'  // 十六位十六进制数作为密钥
const iv = 'contentDocuments'   // 十六位十六进制数作为密钥偏移量
...
class Tools{
	...
	*/
	formatTime(number, format) {
		...
		returnArr.push(this.appZero(parseInt(date.getMonth() + 1), 2));
		returnArr.push(this.appZero(parseInt(date.getDate()), 2));

		returnArr.push(this.appZero(parseInt(date.getHours()), 2));
		returnArr.push(this.appZero(parseInt(date.getMinutes()), 2));
		returnArr.push(this.appZero(parseInt(date.getSeconds()), 2));
	...
	decryptData(word) {
		...
		var decrypt = CryptoJS.AES.decrypt(word, CryptoJS.enc.Utf8.parse(key_hash), {
	...
	encryptData(word) {
		var key_hash = CryptoJS.MD5(key)
		var res = CryptoJS.AES.encrypt(word, CryptoJS.enc.Utf8.parse(key_hash), { iv: CryptoJS.enc.Utf8.parse(iv), mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 })
		return res.toString()
	}
	...
	Transferred (url) {
		if (url.indexOf('http') !== -1) {
			...
			return (__config.fileBasePath + url).replace(/\\/g, '/')
		}
	}

	simpleAddress (e) {
		return (e[0] == e[1] || "重庆" == e[0] ? "" : e[0]) + ("县" == e[1] ? "" : e[1]) + e[2];
	}	
}
